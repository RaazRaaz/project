...
public abstract class ImageScale extends FrameLayout {
    ...
    private final static int SELF_SIZE_DP = 400;
    ...
    public ImageScale(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public ImageScale(Context context, AttributeSet attrs, int defStyle) {
...
//        this.iv_flip.setImageResource(R.drawable.flip);
        ...
        this.iv_delete.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                if(ImageScale.this.getParent()!=null){
                    ViewGroup myCanvas = ((ViewGroup)ImageScale.this.getParent());
                    myCanvas.removeView(ImageScale.this);
                }
            }
        });
            ...
            if(view.getTag().equals("DraggableViewGroup")) {
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        ImageScale.this.setX(ImageScale.this.getX()+offsetX);
                        ImageScale.this.setY(ImageScale.this.getY() + offsetY);
                        move_orgX = event.getRawX();
                        move_orgY = event.getRawY();
                        break;
                ...
                switch (event.getAction()) {
                    case MotionEvent.ACTION_DOWN:
                        ...
                        this_orgX = ImageScale.this.getX();
                        this_orgY = ImageScale.this.getY();

                        scale_orgX = event.getRawX();
                        scale_orgY = event.getRawY();
                        scale_orgWidth = ImageScale.this.getLayoutParams().width;
                        scale_orgHeight = ImageScale.this.getLayoutParams().height;

                        rotate_orgX = event.getRawX();
                        rotate_orgY = event.getRawY();

                        centerX = ImageScale.this.getX()+
                                ((View)ImageScale.this.getParent()).getX()+
                                (float)ImageScale.this.getWidth()/2;
                        ...
                        centerY = ImageScale.this.getY()+
                                ((View)ImageScale.this.getParent()).getY()+
                                statusBarHeight+
                                (float)ImageScale.this.getHeight()/2;
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        if(length2 > length1
                            ...
                            ImageScale.this.getLayoutParams().width += offset;
                            ImageScale.this.getLayoutParams().height += offset;
                            onScaling(true);
                            //DraggableViewGroup.this.setX((float) (getX() - offset / 2));
                            //DraggableViewGroup.this.setY((float) (getY() - offset / 2));
                        }else if(length2 < length1
                                && (angle_diff < 25 || Math.abs(angle_diff-180)<25)
                                && ImageScale.this.getLayoutParams().width > size/2
                                && ImageScale.this.getLayoutParams().height > size/2) {
                            //scale down
                            double offsetX = Math.abs(event.getRawX() - scale_orgX);
                            double offsetY = Math.abs(event.getRawY() - scale_orgY);
                            double offset = Math.max(offsetX, offsetY);
