...
public abstract class ImageHeadScale extends FrameLayout {

    public static final String TAG = "com.knef.ImageScale";
//    private BorderView iv_border;
    ...
    public ImageHeadScale(Context context) {
        super(context);
        init(context);
    }

    public ImageHeadScale(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public ImageHeadScale(Context context, AttributeSet attrs, int defStyle) {
...
//        this.addView(iv_border, iv_border_params);
        ...
        this.iv_delete.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                if(ImageHeadScale.this.getParent()!=null){
                    ViewGroup myCanvas = ((ViewGroup) ImageHeadScale.this.getParent());
                    myCanvas.removeView(ImageHeadScale.this);
                }
            }
        });
            ...
            if(view.getTag().equals("DraggableViewGroup")) {
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        ImageHeadScale.this.setX(ImageHeadScale.this.getX()+offsetX);
                        ImageHeadScale.this.setY(ImageHeadScale.this.getY() + offsetY);
                        move_orgX = event.getRawX();
                        move_orgY = event.getRawY();
                        break;
                ...
                switch (event.getAction()) {
                    case MotionEvent.ACTION_DOWN:
                        ...
                        this_orgX = ImageHeadScale.this.getX();
                        this_orgY = ImageHeadScale.this.getY();

                        scale_orgX = event.getRawX();
                        scale_orgY = event.getRawY();
                        scale_orgWidth = ImageHeadScale.this.getLayoutParams().width;
                        scale_orgHeight = ImageHeadScale.this.getLayoutParams().height;

                        rotate_orgX = event.getRawX();
                        rotate_orgY = event.getRawY();

                        centerX = ImageHeadScale.this.getX()+
                                ((View) ImageHeadScale.this.getParent()).getX()+
                                (float) ImageHeadScale.this.getWidth()/2;
                        ...
                        centerY = ImageHeadScale.this.getY()+
                                ((View) ImageHeadScale.this.getParent()).getY()+
                                statusBarHeight+
                                (float) ImageHeadScale.this.getHeight()/2;
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        if(length2 > length1
                            ...
                            ImageHeadScale.this.getLayoutParams().width += offset;
                            ImageHeadScale.this.getLayoutParams().height += offset;
                            onScaling(true);
                            //DraggableViewGroup.this.setX((float) (getX() - offset / 2));
                            //DraggableViewGroup.this.setY((float) (getY() - offset / 2));
                        }else if(length2 < length1
                                && (angle_diff < 25 || Math.abs(angle_diff-180)<25)
                                && ImageHeadScale.this.getLayoutParams().width > size/2
                                && ImageHeadScale.this.getLayoutParams().height > size/2) {
                            //scale down
                            double offsetX = Math.abs(event.getRawX() - scale_orgX);
                            double offsetY = Math.abs(event.getRawY() - scale_orgY);
                            double offset = Math.max(offsetX, offsetY);
